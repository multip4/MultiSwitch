  ...
  while (1) {
    ...
    if (clone_spec) {
      ...
      if (egress_port >= 0) {
        ...
        BMLOG_DEBUG_PKT(*packet, "Check 3: start parsing");
        parser->parse(packet_copy.get());
        enqueue(egress_port, std::move(packet_copy));
        packet->restore_buffer_state(packet_out_state);
        BMLOG_DEBUG_PKT(*packet, "Check 4: restore buffer state");
...
SimpleSwitch::egress_thread(size_t worker_id) {
  ...
  while (1) {
    ...
    if (clone_spec) {
      ...
      int egress_port = 255; // 255 -> cpu port
      if (egress_port >= 0) {
        f_clone_spec.set(0);
        p4object_id_t field_list_id = clone_spec >> 16;
