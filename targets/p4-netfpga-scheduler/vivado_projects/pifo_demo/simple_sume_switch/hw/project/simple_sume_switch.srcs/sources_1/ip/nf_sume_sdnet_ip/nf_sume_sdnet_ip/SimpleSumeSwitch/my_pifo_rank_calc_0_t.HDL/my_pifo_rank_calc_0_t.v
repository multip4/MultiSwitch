...
`timescale 1 ps / 1 ps
`include "my_cpu_regs_defines.v"
module my_pifo_rank_calc_0_t #(
...
wire     [C_S_AXI_DATA_WIDTH-1:0]          cpu2ip_my_reg_data;
wire     [INDEX_WIDTH-1:0]                 cpu2ip_my_reg_index;
wire                                       cpu2ip_my_reg_valid;
...
(
  ...
  .ip2cpu_my_reg_data          (), //wire
  .ip2cpu_my_reg_index         (), //reg
  .ip2cpu_my_reg_valid         (), //reg
  .ipReadReq_my_reg_index      (),  //wire
  .ipReadReq_my_reg_valid      (), //wire
  // CPU writes IP interface
  .cpu2ip_my_reg_data          (cpu2ip_my_reg_data),
  .cpu2ip_my_reg_index         (cpu2ip_my_reg_index),
  .cpu2ip_my_reg_valid         (cpu2ip_my_reg_valid),
...
case (input_port)
    'b00000001:
        begin
        input_round  = last_pkt_info0 [ROUND_WIDTH + CLASS_WIDTH + PIFO_INFO_WIDTH -1:CLASS_WIDTH + PIFO_INFO_WIDTH];
        reg_port_info = 0;
        end
    'b00000100:
        begin
        input_round  = last_pkt_info1 [ROUND_WIDTH + CLASS_WIDTH +PIFO_INFO_WIDTH -1:CLASS_WIDTH + PIFO_INFO_WIDTH];
        reg_port_info = 1;
        end
    'b00010000:
        begin
        input_round  = last_pkt_info2 [ROUND_WIDTH + CLASS_WIDTH + PIFO_INFO_WIDTH -1:CLASS_WIDTH + PIFO_INFO_WIDTH];
        reg_port_info = 2;
        end
    'b01000000:
        begin
        input_round  = last_pkt_info3 [ROUND_WIDTH + CLASS_WIDTH + PIFO_INFO_WIDTH -1:CLASS_WIDTH + PIFO_INFO_WIDTH];
        reg_port_info = 3;
        end
    default : 
        begin
...
if(cpu2ip_my_reg_valid) //config sig come
    begin
    reg_config_weight_next = cpu2ip_my_reg_data;
    reg_write_address_next = cpu2ip_my_reg_index;  
    reg_round_next = 'd0;
    reg_counter_next = 'd0;
...
assign tuple_out_my_pifo_rank_calc_output_VALID = reg_read_sig_d1;
