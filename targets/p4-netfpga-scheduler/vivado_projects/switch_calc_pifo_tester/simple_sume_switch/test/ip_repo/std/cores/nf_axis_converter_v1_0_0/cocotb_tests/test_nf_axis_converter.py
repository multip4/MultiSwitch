...
def reset_dut(dut):
    ...
    dut.axi_resetn <= 0
    yield ClockCycles(dut.axi_aclk, 10)
    dut.axi_resetn <= 1
...
@cocotb.test()
def test_nf_axis_converter(dut):
    """Test to make sure that nf_axis_converter is working properly.
    """
    # start HW sim clock
    cocotb.fork(Clock(dut.axi_aclk, PERIOD).start())

    yield reset_dut(dut)
    dut.interface_number <= 1
    yield ClockCycles(dut.axi_aclk, START_DELAY)

    # read the pkts and rank values
    pkts_in, meta_in = make_pkts_meta_in()

    # Attach an AXI4Stream Master to the input pkt interface
    pkt_master = AXI4StreamMaster(dut, 's_axis', dut.axi_aclk)

    # Attach an AXI4StreamSlave to the output pkt interface
    pkt_slave = AXI4StreamSlave(dut, 'm_axis', dut.axi_aclk, tready_delay=BP_COUNT, idle_timeout=IDLE_TIMEOUT)
    ...
    yield ClockCycles(dut.axi_aclk, 20)
