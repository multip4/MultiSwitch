...
from os import remove,fdopen, remove
...
def find_stim_axi_files(path_of_stim_axi):
    nf_stim_files = []
    for file in os.listdir(path_of_stim_axi):
        if file.startswith("nf_interface") and file.endswith("stim.axi"):
            nf_stim_files.append(path_of_stim_axi+'/'+file)
    return nf_stim_files

def modify2burst_axis(path_of_stim_axi_file):
     #Create temp file
    fh, abs_path = mkstemp()
    with fdopen(fh,'w') as new_file:
        with open(path_of_stim_axi_file) as old_file:
            for line in old_file:
                if "@" in line:
                    new_file.write(line.replace(line, "@    10000\n"))
                elif "+" in line:
                    new_file.write(line.replace(line, "+    0\n"))
                else:
                    new_file.write(line)

    #Copy the file permissions from the old file to the new file
    copymode(path_of_stim_axi_file, abs_path)
    #Remove original file
    remove(path_of_stim_axi_file)
    #Move new file
    move(abs_path, path_of_stim_axi_file)
...
def prepareTestWorkDir(testName):
    ...
    # forest.1000 modify part.
    for file in find_stim_axi_files(src_test_dir):
        modify2burst_axis(file)
