...
typedef bit<32> TimeWidth_t;
typedef bit<32> PIFO_t;
...
parser TopParser(packet_in b, 
    ...
    state start {
        ...
        user_metadata.rc_class_weight = 2;
...
*/
    ...
    action set_rc_class(bit<CLASS_WIDTH> class_id){
    ...
    action set_rc_weight(bit<WEIGHT_WIDTH> weight){
        user_metadata.rc_class_weight = weight;
    }
    table table_rc_weight_lookup {
        key = { p.ethernet.srcAddr: exact; }
        actions = {
            set_rc_weight;
        }
    ...
    apply {
        ...
        p.int_h.pifo_info = sume_metadata.pifo_info;
    }
}
