...
//    output                                    control_S_AXI_AWREADY
    ...
    assign {w_req_class_id, w_req_class_weight, w_req_pkt_size, w_req_calc_type, w_req_output_port} = tuple_in_@EXTERN_NAME@_input_DATA

    wire                      w_req_wrr_valid;
    assign w_req_wrr_valid = tuple_in_@EXTERN_NAME@_input_VALID & w_req_calc_type[1];
    wire                      w_req_drr_valid;
    assign w_req_drr_valid = tuple_in_@EXTERN_NAME@_input_VALID & w_req_calc_type[2];
    wire                      w_req_wfq_valid;
    assign w_req_wfq_valid = tuple_in_@EXTERN_NAME@_input_VALID & w_req_calc_type[3];
...
// WRR ENGINE
    ...
    #(
      ...
      .PIFO_RANK_WIDTH(PIFO_RANK_WIDTH),
    ...
    wrr_inst(
      ...
      .last_pifo_rank(w_last_pkt_round),
      .resp_valid(w_wrr_out_valid),
      .resp_data(w_wrr_out_data),
      .clk(clk_lookup),
      .rstn(clk_lookup_rst_high)
    );
              ...
              if (~empty_fifo) 
                begin
                  ...
                  case(w_output_port_bit_array)
                    PORT0:
                      begin
                        r_last_pkt_info_next = last_pkt_info0;
                      end
                    PORT1:
                      begin
                        r_last_pkt_info_next = last_pkt_info1;
                      end
                    'PORT2:
                      begin
                        r_last_pkt_info_next = last_pkt_info2;
                      end
                    PORT3:
                      begin
                        r_last_pkt_info_next = last_pkt_info3;
                      end
                    DMA:
                      begin
                        r_last_pkt_info_next = last_pkt_info4;
                      end
                  endcase
